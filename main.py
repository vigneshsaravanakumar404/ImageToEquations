...
for segment in path:
    ...
    if isinstance(segment, svgpathtools.path.Line):
        ...
        equations.append("\\\\left((1-t)^3*" + str(p0.real) + "+3*t*(1-t)^2*" + str(p1.real) + "+3*t^2*(1-t)*" + str(
            p2.real) + "+t^3*" + str(p3.real) + ", (1-t)^3*" + str(p0.imag) + "+3*t*(1-t)^2*" + str(
            p1.imag) + "+3*t^2*(1-t)*" + str(p2.imag) + "+t^3*" + str(p3.imag) + ")\\\\right)")

        # Convert the bezier points into a parametric equation in lambda format
        regularEquations.append(
            lambda t: (1 - t) ** 3 * p0 + 3 * t * (1 - t) ** 2 * p1 + 3 * t ** 2 * (1 - t) * p2 + t ** 3 * p3)
        ...
        # Convert the bezier points into a parametric equation in latex format
        equations.append("\\\\left((1-t)^2*" + str(p0.real) + "+2*t*(1-t)*" + str(p1.real) + "+t^2*" + str(
            p2.real) + ", (1-t)^2*" + str(p0.imag) + "+2*t*(1-t)*" + str(p1.imag) + "+t^2*" + str(
            p2.imag) + ")\\\\right))")
        ...
        # Convert the bezier points into a parametric equation in latex format
        equations.append("\\\\left(" + str(p0.real) + "+" + str(r.real) + "*\\cos(t), " + str(p0.imag) + "+" + str(
            r.imag) + "*\\sin(t)\\\\right)")
...
<div id="calculator" style="width: window.innerWidth; height: window.innerHeight;"></div>
<script>
  var elt = document.getElementById('calculator');
  var calculator = Desmos.GraphingCalculator(elt);
"""

# find the max and min x and y values based on the lambda equations and set the Desmos API bounds
xMin = min([min([equation(t).real for t in np.linspace(0, 1, 1000)]) for equation in regularEquations])
xMax = max([max([equation(t).real for t in np.linspace(0, 1, 1000)]) for equation in regularEquations])
print("xMin: " + str(xMin))
print("xMax: " + str(xMax))
desmos += "calculator.setMathBounds({ left: -500, right: " + str(xMax) + ", bottom: -500, top: " + str(xMax) + " });\n"

# Add each equation to the Desmos API script
for i in range(len(equations)):
    desmos += "calculator.setExpression({ id: 'a-slider" + str(i) + "', latex: '" + equations[i] + "' });\n"

# Close the Desmos API script
desmos += "</script>"
...
# Open the Desmos file in the browser full screen
webbrowser.open("desmos.html", new=2)

# TODO:
# Create demo video
# Create tkinter GUI
# Make the demos output look better
# Explain the math 
# Comment the code

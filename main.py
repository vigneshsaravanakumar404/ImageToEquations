...
using complex numbers. Convert the bezier equations to graph able parametrics. 
Put them in latex format and then use the desmos API to create a viewable
...
for segment in path:
    ...
    if isinstance(segment, svgpathtools.path.Line):
        ...
        if end.real - start.real != 0:
            ...
            equations.append("y=" + str(m) + "x+" + str(b) + "\\left\\{" + str(xMin) + "\\le x\\le " + str(
                xMax) + "\\right\\}\\left\\{" + str(yMin) + "\\le y\\le " + str(yMax) + "\\right\\}")
